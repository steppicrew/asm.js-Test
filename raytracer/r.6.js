...
//    "use asm";
    ...
    var SIZEOF_INT32= stdlib.Int32Array.BYTES_PER_ELEMENT;
...
//    var heap8= heap;
    ...
    function closest_intersection(Bx, By, Bz, Dx, Dy, Dz, t_min, t_max) {
        ...
        for ( ; r = inHeap[inx_spheres + (q|0)]|0; q= (q|0) + 9) {  
            // Compute quadratic equation coefficients K1, K2, K3

            Jx = +(Bx - +(inHeap[inx_spheres + q + (1|0)] >> 0));  // origin - center
            Jy = +(By - +(inHeap[inx_spheres + q + (2|0)] >> 0));  // origin - center
            Jz = +(Bz - +(inHeap[inx_spheres + q + (3|0)] >> 0));  // origin - center
...
console.log("s", s);
        ...
        Nx = Xx - inHeap[inx_spheres + s];
        Ny = Xy - inHeap[inx_spheres + s + 1];
        Nz = Xz - inHeap[inx_spheres + s + 2];
        ...
        // For each light
        for (l = 0; u = inHeap[inx_lights + l]; l = l + 4) { // Get intensity and check for end of array

            // Compute vector from intersection to light (L = lights[l++] - X) and
            // k = <N,L> (reused below)
            // L = A_minus_Bk(lights[l++], Xx, Xy, Xz, 1);
            Lx = inHeap[inx_lights + l + 1] - Xx;
            Ly = inHeap[inx_lights + l + 2] - Xy;
            Lz = inHeap[inx_lights + l + 3] - Xz;
            ...
            i += u * 
                !cc * (
                    ...
                    + max(0, pow(dot(Mx, My, Mz, Dx, Dy, Dz) 
                        / sqrt(dot(Mx, My, Mz, Mx, My, Mz) * dot(Dx, Dy, Dz, Dx, Dy, Dz)), inHeap[inx_spheres + s + 6]))
                );
        }
        ...
        var local_color = inHeap[inx_spheres + s + c + 2] * i * 2.8;
        ...
        var ref = inHeap[inx_spheres + s + 7] / 9;
    ...
    function run( heapIndex ) {
        ...
        w = inHeap[inHeap[heapIndex]|0]|0;

        // Sphere: radius, [cx,  cy,  cz], R,  G,  B, specular exponent, reflectiveness 
        // R, G, B in [0, 9], reflectiveness in [0..9].
        inx_spheres= inHeap[(heapIndex|0) + 1]|0;

        // Ambient light.
        ambient_light = inHeap[inHeap[(heapIndex|0) + 2]|0]|0;

        // Point lights: intensity, [x,  y,  z]
        // Intensities should add to 10, including ambient.
        inx_lights = inHeap[(heapIndex|0) + 3]|0;

        inx_output= inHeap[(heapIndex|0) + 4]|0;

        var y, h, x;

        var out_start= (inx_output + 1) * SIZEOF_INT32;
...
// window.ct= window.ct ? window.ct + 1 : 1; console.log(window.ct, c, x, y, w, h);
                    outHeap[out_idx++] = trace_ray(0, 1, 0, x / w, y / w, 1, 1, w, 2);
                }
                outHeap[out_idx++] = 255; // Opaque alpha
            }
        }

        inHeap[inx_output]= out_idx - out_start;
